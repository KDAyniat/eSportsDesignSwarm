...
<div id="pageMiddle">
    ...
    $sub_query = mysqli_query($db_conx, "SELECT job_id FROM submission WHERE submitted_by='.$u'  ");
    $numrows = mysqli_num_rows($sub_query);
    $i= 0;
    while($i < $numrows){
    $row=mysqli_fetch_array($sub_query,MYSQLI_ASSOC);
    $job_id = $row[0];
    $jobs_query = mysqli_query($db_conx, "SELECT * FROM jobs WHERE id='$job_id'");
